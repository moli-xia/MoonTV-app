...
function LoginPageClient() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      // 优先使用 RUNTIME_CONFIG，如果不存在则使用环境变量
      const storageType = (window as any).RUNTIME_CONFIG?.STORAGE_TYPE || 
                         process.env.NEXT_PUBLIC_STORAGE_TYPE || 'localstorage';
      const enableRegisterEnv = (window as any).RUNTIME_CONFIG?.ENABLE_REGISTER ||
                               process.env.NEXT_PUBLIC_ENABLE_REGISTER === 'true';
      
      setShouldAskUsername(storageType && storageType !== 'localstorage');
      setEnableRegister(Boolean(enableRegisterEnv));
    }
  }, []);
    ...
    <Suspense
      fallback={
        <div className='min-h-screen flex items-center justify-center px-4'>
          <div className='w-full max-w-md rounded-3xl bg-white/60 dark:bg-zinc-900/60 backdrop-blur-xl shadow-2xl p-10 dark:border dark:border-zinc-800'>
            <div className='h-8 w-2/3 mx-auto bg-gray-200/70 dark:bg-zinc-800/70 rounded animate-pulse mb-8' />
            <div className='space-y-4'>
              <div className='h-12 w-full bg-gray-200/70 dark:bg-zinc-800/70 rounded-lg animate-pulse' />
              <div className='h-12 w-full bg-gray-200/70 dark:bg-zinc-800/70 rounded-lg animate-pulse' />
              <div className='h-12 w-full bg-gray-200/70 dark:bg-zinc-800/70 rounded-lg animate-pulse' />
            </div>
          </div>
