        ...
        <div className='flex items-center justify-between'>
          ...
          <button
            ...
            className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 ${
              ...
              isD1Storage || isUpstashStorage ? 'opacity-50 cursor-not-allowed' : ''
            }`}
          >
            <span
    ...
    const result = await Swal.fire({
      ...
      html: `
        <div style="display:flex;flex-direction:column;gap:12px;text-align:left;">
          <div id="edit-source-key" style="font-size:12px;color:#6b7280;"></div>
          <input id="edit-source-name" class="swal2-input" placeholder="名称" />
          <input id="edit-source-api" class="swal2-input" placeholder="API 地址" />
          <input id="edit-source-detail" class="swal2-input" placeholder="Detail 地址（选填）" />
        </div>
      `,
      focusConfirm: false,
      showCancelButton: true,
      ...
      didOpen: () => {
        ...
        const keyEl = popup?.querySelector(
          '#edit-source-key'
        ) as HTMLDivElement | null;
        const nameEl = popup?.querySelector(
          '#edit-source-name'
        ) as HTMLInputElement | null;
        const apiEl = popup?.querySelector(
          '#edit-source-api'
        ) as HTMLInputElement | null;
        const detailEl = popup?.querySelector(
          '#edit-source-detail'
        ) as HTMLInputElement | null;
        if (keyEl) keyEl.textContent = `Key：${source.key}`;
        if (nameEl) nameEl.value = source.name || '';
        if (apiEl) apiEl.value = source.api || '';
        if (detailEl) detailEl.value = source.detail || '';
      },
      preConfirm: () => {
        const popup = Swal.getPopup();
        const nameEl = popup?.querySelector(
        ...
        if (!name || !api) {
          Swal.showValidationMessage('名称与 API 地址不能为空');
          return;
        }

        return { name, api, detail };
      },
    });

    if (!result.isConfirmed) return;
    const value = result.value as { name: string; api: string; detail: string };
    await callSourceApi({
      action: 'update',
      key: source.key,
      name: value.name,
        ...
        <td className='px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2'>
          <button
            ...
            className={`inline-flex items-center px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${
              'bg-blue-100 text-blue-800 hover:bg-blue-200 dark:bg-blue-900/40 dark:hover:bg-blue-900/60 dark:text-blue-200'
            }`}
          >
            编辑
          </button>
        ...
        <div className='p-4 bg-gray-50 dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700 space-y-4'>
          ...
          <div className='flex justify-end'>
            <button
              ...
              disabled={
                !newSource.name || !newSource.key || !newSource.api
              }
              className='w-full sm:w-auto px-4 py-2 bg-green-600 hover:bg-green-700 disabled:bg-gray-400 text-white rounded-lg transition-colors'
            >
              添加
...
function AdminPageClient() {
  ...
  const handleResetConfig = async () => {
    if (isLocalStorage) {
      showError('localstorage 模式不支持管理端重置配置');
      return;
    }
    const { isConfirmed } = await Swal.fire({
      title: '确认重置配置',
      ...
      <div className='px-2 sm:px-10 py-4 sm:py-8'>
        <div className='max-w-[95%] mx-auto'>
          ...
          <div className='flex items-center gap-2 mb-8'>
            ...
            {config && role === 'owner' && !isLocalStorage && (
              <button
          ...
          {isLocalStorage && (
            <div className='mb-6 rounded-lg border border-yellow-200 dark:border-yellow-800 bg-yellow-50 dark:bg-yellow-900/20 px-4 py-3 text-sm text-yellow-800 dark:text-yellow-200'>
              localstorage 模式下修改会写入 config.json
            </div>
          )}
