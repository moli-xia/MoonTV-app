...
- 👿 **智能去广告**：自动跳过视频中的切片广告（实验性）
  ...
  <img src="https://raw.githubusercontent.com/moli-xia/MoonTV-app/main/public/phone.png" alt="项目截图" style="max-width:200px">
  <br>
...
localstorage 模式支持管理页面与站点/用户/视频源配置（写入 `config.json`），但不支持多账户与跨设备同步；如需多账户与同步请使用 redis/d1/upstash。
...
```bash
# 拉取预构建镜像
docker pull superneed/moontv-amd:latest
# ARM64 服务器请使用：
# docker pull superneed/moontv-arm:latest

# 运行容器
# -d: 后台运行  -p: 映射端口 3000 -> 3000
docker run -d --name moontv -p 3000:3000 --env PASSWORD=your_password superneed/moontv-amd:latest
# ARM64 服务器请使用：
# docker run -d --name moontv -p 3000:3000 --env PASSWORD=your_password superneed/moontv-arm:latest
```

数据分离与持久化（推荐）：将 `config.json` 挂载到宿主机，便于升级/重建容器且保留后台配置。

```bash
# 选择一个宿主机目录存放数据
mkdir -p /opt/moontv

# 首次初始化 config.json（从镜像内拷贝一份到宿主机）
docker run --rm superneed/moontv-amd:latest sh -c 'cat /app/config.json' > /opt/moontv/config.json
# ARM64 服务器请使用：
# docker run --rm superneed/moontv-arm:latest sh -c 'cat /app/config.json' > /opt/moontv/config.json

# 使用挂载的 config.json 运行（可写，后台修改会写回宿主机）
docker rm -f moontv >/dev/null 2>&1 || true
docker run -d --name moontv \
  --restart unless-stopped \
  -p 3000:3000 \
  -e PASSWORD=your_password \
  -v /opt/moontv/config.json:/app/config.json \
  superneed/moontv-amd:latest
# ARM64 服务器请将镜像替换为 superneed/moontv-arm:latest
```
...
services:
  moontv:
    image: superneed/moontv-amd:latest # ARM64 服务器请改为 superneed/moontv-arm:latest
    ...
    # 如需自定义配置/后台保存配置，建议挂载可写 config.json
    # volumes:
...
localstorage 模式下访问 `/admin` 默认视为站长（owner），可修改站点配置、用户配置、视频源配置，并写入 `config.json`（Docker 部署建议将 `config.json` 挂载为可写以便持久化）。

非 localstorage 部署：设置环境变量 USERNAME 和 PASSWORD 即为站长用户，站长可设置用户为管理员
